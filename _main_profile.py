...
# shutdown event and py-spy process handles
...
async def startup():
    ...
    global py_spy_main_proc, py_spy_merlin_proc
    merlin.start()
...
def signal_handler(sig, frame):
    ...
    if merlin.is_alive():
        ...
        if merlin.is_alive():
            print("Warning: Merlin process did not terminate gracefully. Forcing.")
            merlin.terminate() # Force if join times out
    sys.exit(0)
